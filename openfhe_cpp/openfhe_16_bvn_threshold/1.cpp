...
int main(int argc, char *argv[]) {
    ...
    std::vector<int64_t> data1 = split(s1);
    std::vector<int64_t> data2 = split(s2);
    std::vector<int64_t> data3 = split(s3);

    Plaintext d1 = cc->MakePackedPlaintext(data1);
    Plaintext d2 = cc->MakePackedPlaintext(data2);
    Plaintext d3 = cc->MakePackedPlaintext(data3);
