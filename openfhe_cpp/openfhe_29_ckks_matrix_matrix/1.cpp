...
for (int i = -3; i <= 3; i++) rotationKeys.push_back(i);
cryptoContext->EvalRotateKeyGen(keyPair.secretKey, rotationKeys);

std::cout << "Done"<< std::endl << std::endl;


std::vector<double> vector = genRandVect(n1, max,0);
Plaintext vectorP  = cryptoContext->MakeCKKSPackedPlaintext(vector);

auto matrix1 = genRandMatrix(n1, n2, max,1);
auto matrix2 = genRandMatrix(n2, n3, max,2);
...
std::cout << "V1*M1*M2  = " << resOutput << std::endl;
...
std::cout << "VectorC*Matrix*Matrix (by inner product)      = " << res->GetCKKSPackedValue()  << std::endl;
